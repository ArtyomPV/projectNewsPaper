...
class RegisterView(CreateView):
   ...
   Для внесения дополнительных изменений на этапе создания записи в БД
   следует переопределить метод form_valid как показано ниже:
   '''

    def form_valid(self, form):
        user = form.save()
        # group = Group.objects.get(name='my_group') # Обращаемся к БД,
        # находим нужную группу. Может оказаться, что такой группы в БД нет.
        ...
        group = Group.objects.get_or_create(name='my_group')
